...
void __fastcall TfMain::lPlayClick(TObject *Sender)
{
        initializare_joc=new JOC();  //Instantiem obiectul joc
         ...
         initializare_joc->Piese_Luate();//Pune label unde sa punem piesele pe care le iei
...
void __fastcall TfMain::csClientRead(TObject *Sender,
...
void __fastcall TfMain::lConnectClick(TObject *Sender)
{

        String text;
        if(InputQuery("Conectare", "Scrie adresa ip", text)){
        ShowMessage("Ati introdus adresa ip: " + text + " si ati apasat OK");
        csClient->Address=text;
...
void __fastcall TfMain::lConnectMouseEnter(TObject *Sender)
{
        lConnect->Font->Color=clYellow;
        lConnect->Font->Size+=6;
        sndPlaySound("buton.wav", SND_ASYNC);
}
//---------------------------------------------------------------------------

void __fastcall TfMain::lConnectMouseLeave(TObject *Sender)
{
        lConnect->Font->Color=clWhite;
        lConnect->Font->Size=24;
}
//---------------------------------------------------------------------------
